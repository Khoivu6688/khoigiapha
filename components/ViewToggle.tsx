...
// Bổ sung "print_config" vào ViewMode
...
export default function ViewToggle({ isAdmin }: { isAdmin?: boolean }) {
  ...
  return (
    <div className="flex items-center w-fit max-w-[98vw] mx-auto overflow-x-auto no-scrollbar bg-white/80 p-1.5 rounded-full shadow-sm mt-6 mb-4 relative border border-stone-200/60 backdrop-blur-md z-50 transition-all justify-start sm:justify-center whitespace-nowrap px-2">
      ...
      {/* NÚT PRINT A0 Ở CUỐI - CHỈ HIỆN CHO ADMIN */}
      {isAdmin && (
        <button
          onClick={() => setView("print_config" as any)}
          className={`relative flex-shrink-0 px-4 sm:px-6 py-1.5 sm:py-2.5 text-xs sm:text-sm font-bold rounded-full transition-colors duration-300 z-10 flex items-center gap-2 border-l border-stone-200 ml-2 ${
            currentView === "print_config" ? "text-amber-700" : "text-amber-600 hover:text-amber-800"
          }`}
        >
