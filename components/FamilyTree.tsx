...
export default function FamilyTree({
  ...
  // LOGIC SẮP XẾP VÀ XỬ LÝ DỮ LIỆU
  const getTreeData = (personId: string) => {
    ...
    const childrenList = (childRels.map((r) => personsMap.get(r.person_b)).filter(Boolean) as Person[])
      .sort((a, b) => {
        ...
        // 1. Nếu cả hai đều có số thứ tự: Xếp tăng dần (6 -> 7 -> 8)
        if (aOrder !== null && bOrder !== null) {
          return aOrder - bOrder;
        }
        // 2. Nếu A có số, B null: Đẩy A lên trước (bên trái)
        if (aOrder !== null && bOrder === null) return -1;
        // 3. Nếu A null, B có số: Đẩy B lên trước (bên trái)
        if (aOrder === null && bOrder !== null) return 1;

        // 4. Nếu cả hai đều null: Xếp theo năm sinh
        ...
        <div className="node-container inline-flex flex-col items-center">
          <div className="flex relative z-10 bg-white rounded-2xl shadow-md border border-stone-200/80">
            ...
            {data.spouses.map((spouseData, idx) => (
              <FamilyNodeCard key={spouseData.person.id} isRingVisible={idx === 0} person={spouseData.person} role={spouseData.person.gender === "male" ? "Chồng" : "Vợ"} note={spouseData.note} onClickSetRoot={() => setRootId(spouseData.person.id)} />
            ))}
            {hasChildren && (
              <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-white border border-stone-200/80 rounded-full size-6 flex items-center justify-center shadow-md z-10 cursor-pointer text-stone-500 hover:text-amber-600" 
  ...
  const handleMouseDown = (e: React.MouseEvent) => { setIsPressed(true); hasDraggedRef.current = false; setDragStart({ x: e.pageX, y: e.pageY }); if (containerRef.current) setScrollStart({ left: containerRef.current.scrollLeft, top: containerRef.current.scrollTop }); };
  const handleMouseMove = (e: React.MouseEvent) => { if (!isPressed || !containerRef.current) return; const dx = e.pageX - dragStart.x; const dy = e.pageY - dragStart.y; if (!hasDraggedRef.current && (Math.abs(dx) > 5 || Math.abs(dy) > 5)) { setIsDragging(true); hasDraggedRef.current = true; } if (hasDraggedRef.current) { e.preventDefault(); containerRef.current.scrollLeft = scrollStart.left - dx; containerRef.current.scrollTop = scrollStart.top - dy; } };
  const handleMouseUpOrLeave = () => { setIsPressed(false); setIsDragging(false); };

  return (
    <div className="w-full h-full relative">
      {depthPortalNode && createPortal(<div className="flex items-center gap-2 bg-white/80 backdrop-blur-md shadow-sm border border-stone-200/60 rounded-full px-3 h-10 transition-opacity"><span className="text-sm font-semibold text-stone-600">Độ sâu:</span><input type="number" min="0" value={maxDepth === 0 ? "" : maxDepth} onChange={(e) => setMaxDepth(e.target.value ? parseInt(e.target.value) : 0)} className="w-10 bg-transparent text-sm font-medium text-amber-700 focus:outline-none text-center" placeholder="∞" /></div>, depthPortalNode)}
      {portalNode && createPortal(<div className="flex items-center gap-2 w-max" ref={filtersRef}><div className="flex items-center bg-white/80 backdrop-blur-md shadow-sm border border-stone-200/60 rounded-full overflow-hidden h-10"><button onClick={handleZoomOut} className="px-3 h-full hover:bg-stone-100/50 text-stone-600"><ZoomOut className="size-4" /></button><button onClick={handleResetZoom} className="px-2 h-full text-xs font-medium min-w-[50px] border-x border-stone-200/50">{Math.round(scale * 100)}%</button><button onClick={handleZoomIn} className="px-3 h-full hover:bg-stone-100/50 text-stone-600"><ZoomIn className="size-4" /></button></div><button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 px-4 h-10 rounded-full bg-white/80 text-stone-600 border border-stone-200/60 font-semibold text-sm shadow-sm backdrop-blur-md"><Filter className="size-4" /> Lọc</button></div>, portalNode)}
      ...
      <div ref={containerRef} className={`w-full h-full overflow-auto bg-stone-50 ${isPressed ? "cursor-grabbing" : "cursor-grab"}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUpOrLeave} onMouseLeave={handleMouseUpOrLeave}>
        <div className="w-max min-w-full mx-auto p-4 css-tree" style={{ transform: `scale(${scale})`, transformOrigin: "top center" }}>
